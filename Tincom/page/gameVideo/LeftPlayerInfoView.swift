...
struct LeftPlayerInfoView: View {
    ...
    var body: some View {
        ...
        .opacity(isShow ? 1 : 0)
        .padding(.leading, 100) // 预留空间给退出动画
        .padding(.trailing, 10) // 预留一点边界给回弹动画
...
#Preview {
    let payer = GameVideoVM().getBlueTeamList()[0]
    return LeftPlayerInfoView(index: 0, onShowEvent: .constant(true), playerInfo: payer)
}
